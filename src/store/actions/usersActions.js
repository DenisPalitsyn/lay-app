...
const getMainPhotoUrl = async (user, size) => {
...
export const getUserPhoto = (uid, size = 'BIG') => async (dispatch, getState) => {
  ...
  const photo = await getMainPhotoUrl(user, size);
...
//
// export const getUsersPhoto =
// (usersId: Array<string>) => async (dispatch: any, getState: any, getFirebase: any) => {
...
//   !newRequest.chats.includes(profileUid)
//   && (newRequest.chats = [...newRequest.chats, profileUid]);
//
